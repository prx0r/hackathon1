...
class Workspace:
    def __init__(self, root: str | Path = ".patala-ci"):
        self.root = Path(root)
        for d in ("analyses", "snapshots", "claims", "diffs", "impacts", "obligations", "plans", "receipts", "exports", "mcp_traces", "lineages", "bindings", "tasks", "adjudications", "checkpoints", "attestations"):
    ...
    def save_lineage(self, graph_id: str, graph) -> Path:
        return self._write("lineages", graph_id, graph)

    def load_lineage(self, graph_id: str):
        from .lineage import LineageGraph
        return LineageGraph.from_dict(self._read("lineages", graph_id))

    def save_binding(self, binding) -> Path:
        return self._write("bindings", binding.binding_id, binding)

    def save_task(self, task) -> Path:
        return self._write("tasks", task.task_id, task)

    def save_adjudication(self, adjudication) -> Path:
        return self._write("adjudications", adjudication.adjudication_id, adjudication)

    def save_checkpoint(self, checkpoint_id: str, checkpoint) -> Path:
        return self._write("checkpoints", checkpoint_id, checkpoint)

    def save_attestation(self, attestation_id: str, envelope) -> Path:
        data = envelope.to_dict() if hasattr(envelope, "to_dict") else envelope
        return self._write("attestations", attestation_id, data)
