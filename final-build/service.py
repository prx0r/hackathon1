...
class ResearchCI:
    ...
    def track(self, analysis_id: str, title: str, query: QuerySpec,
        ...
        # A first observation must be a trustworthy baseline. Source failure or
        # partial enrichment can be recorded as events, but must not become the
        # baseline against which later Graph state is interpreted as mass change.
        if snapshot.source_status != SourceStatus.OK.value:
            self.ws.save_snapshot(snapshot)
            self.ws.ledger.append("analysis.track_blocked", analysis_id, {
                "snapshot_id": snapshot.snapshot_id,
                "source_status": snapshot.source_status,
                "source_error": snapshot.source_error,
            })
            raise RuntimeError(f"cannot establish baseline: source_status={snapshot.source_status}")
        self.ws.save_snapshot(snapshot)
        claim_ids = []
