...
class E2ETests(unittest.TestCase):
    ...
    def test_failed_initial_fetch_cannot_be_baseline(self):
        from patala_research_ci.openaire import FetchResult
        class BadStack:
            def fetch(self, spec):
                return FetchResult('UNAVAILABLE',[],[],{},[],'timeout')
        with tempfile.TemporaryDirectory() as td:
            ws=Workspace(td); ci=ResearchCI(ws, BadStack())
            with self.assertRaises(RuntimeError):
                ci.track('a','A',QuerySpec(search='x'),[])
            self.assertEqual(ws.list_analyses(),[])
            self.assertEqual(ws.ledger.events()[-1]['event_type'],'analysis.track_blocked')
