...
class DiffImpactTests(unittest.TestCase):
    ...
    def test_partial_enrichment_keeps_observed_relation_additions(self):
        partial=Snapshot.from_dict(self.old.to_dict())
        partial.snapshot_id='partial-add'
        partial.source_status='PARTIAL'
        partial.source_error='one ScholeXplorer lookup failed'
        partial.relations=list(self.old.relations)+[{"source":"doi:NEW","relation":"Cites","target":"doi:T","subtype":""}]
        d=diff_snapshots('a',self.old,partial)
        self.assertTrue(any(c.kind=='RELATION_ADDED' for c in d.changes))
        self.assertFalse(any(c.kind=='RELATION_REMOVED' for c in d.changes))
