...
class MCPTraceTests(unittest.TestCase):
    ...
    def test_bundled_live_trace_is_real_and_contains_no_obvious_secrets(self):
        root=Path(__file__).resolve().parents[1]
        trace=load_trace(root/'artifacts'/'alien_mcp_trace.live.json')
        self.assertFalse(trace.synthetic)
        self.assertGreater(len(trace.calls),0)
        self.assertGreater(len(trace.openaire_ids),0)
        raw=json.dumps(trace.to_dict()).lower()
