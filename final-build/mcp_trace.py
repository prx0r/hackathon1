...
SENSITIVE_KEYS = {
    ...
    "x-api-key", "access-token", "client_secret", "private_key", "bearer",
}
...
def validate_trace(trace: MCPTrace) -> None:
    ...
    for call in trace.calls:
        ...
        # Fail closed if obvious bearer/JWT-like credentials survive sanitization.
        if "bearer " in encoded:
            raise ValueError("MCP trace contains bearer credential material")
        jwt = re.compile(r"eyJ[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}")
        if jwt.search(encoded):
            raise ValueError("MCP trace contains JWT-like credential material")
