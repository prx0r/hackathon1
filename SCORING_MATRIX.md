...
|---|---|---:|---|
| **1. AI MCP connector using OpenAIRE Graph** | Dual-MCP architecture; trace capture/redaction/digest/binding; companion MCP; 4 trace tests; official connector named as required AI discovery plane. Alien's existing features (auto-updates, audit trails, redaction) are explicitly acknowledged; Pāṭala's contribution is the downstream dependency/impact layer Alien does not have. | **4/5 in packaged artifact; 5/5 after one real Alien trace is captured** | Build environment has no participant Alien credentials. Bundled trace is intentionally synthetic. |
